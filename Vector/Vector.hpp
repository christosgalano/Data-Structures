...
template <typename T>
Vector<T>::Vector() 
    : sz{}, cap{min_cap}, data{new T[cap]} {}


template <typename T>
Vector<T>::Vector(std::size_t size)
    : sz{size}, cap{ sz < min_cap ? min_cap : sz }, data{new T[cap]} {}
...
Vector<T>::Vector(const std::initializer_list<T>& values)
    : sz{values.size()}, cap{ sz < min_cap ? min_cap : sz }, data{new T[cap]}
{
    std::size_t i {};
    for (auto v : values)
        data[i++] = v;
...
Vector<T>::Vector(const Vector<T>& vec) 
    : sz{vec.sz}, cap{vec.cap}, data{new T[cap]}
{
